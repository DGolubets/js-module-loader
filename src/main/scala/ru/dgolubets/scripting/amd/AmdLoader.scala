...
import ru.dgolubets.scripting.commonjs.internal.exceptions.UnexpectedModuleState
import ru.dgolubets.scripting.internal.ScriptEngineExtensions._
import ru.dgolubets.scripting.internal.{Module, ModuleInstance, ModuleList}
...
class AmdLoader(scriptEngine: NashornScriptEngine, moduleReader: ScriptModuleReader)
   ...
   * @return
   */
  private[amd] def requireLocal(moduleId: String)(implicit loaderContext: AmdLoaderContext): AnyRef = {
   ...
   * @return
   */
  private[amd] def define(moduleId: Option[String], dependencies: Seq[String], moduleFactory: JSObject)(implicit loaderContext: AmdLoaderContext): Unit = {

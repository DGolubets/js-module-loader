...
import ru.dgolubets.internal.StateMachine

import scala.concurrent._

private[scripting] object Module {
   ...
   */
  sealed case class Loaded[T <: ModuleDefinition](definition: T) extends NoErrorState
 ...
 * @tparam T Module definition type
 */
private[scripting] case class Module[T <: ModuleDefinition](id: String) extends StateMachine[Module.State](Module.Empty()) {
  import Module._
  ...
  override protected def onStateChanged(newState: State, oldState: State): Unit = {
    ...
    newState match {
      case _: Loaded[T] => _loaded.success(())
   ...
   * @return
   */
  def tryLoad(definition: T): Boolean = tryTransition[Loading, Loaded[T]](Loaded(definition))

  /**
   * Sets the module to an Loaded state.
   * @param definition Module definition
   */
  def load(definition: T) = throwInvalidState(tryLoad(definition))
   ...
   * @return
   */
  def tryStartInitializing(instance: ModuleInstance): Boolean = tryTransition[Loaded[T], Initializing](Initializing(instance))
